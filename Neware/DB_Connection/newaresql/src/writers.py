...
class CSVWriter(FileHandler):
    ...
    def write(self, data):
        self.makeroot()
        data.to_csv(self.path, mode='w', header=True, index=False)
        return
...
class HDFWriter(FileHandler):
    ...
    def write(self, data, key='data'):
        self.makeroot()
        data.to_hdf(self.path, key, mode='w', append=False,
                    format='table', index=None, data_columns=None, complevel=9)
        return
    ...
    def append(self, data, key='data'):
        if not self.exists:
            ...
            data.to_hdf(self.path, key, mode='r+', append=True,
                        format='table', index=None, data_columns=None, complevel=9)
        return
...
class LaTeXWriter(FileHandler):
    ...
    def read(self, columns=None):
        if isinstance(columns, str):
            columns = [columns]
        data = pd.read_csv(self.path, skiprows=2, skipfooter=2,
                           delimiter='&', engine='python').drop(0, axis=0)
